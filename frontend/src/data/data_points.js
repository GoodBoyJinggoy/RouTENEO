import gate1_1 from "../assets/pictures/gate1/gate1_1.png";
import gate1_2 from "../assets/pictures/gate1/gate1_2.png";
import gate1_3 from "../assets/pictures/gate1/gate1_3.png";

import faura_1 from "../assets/pictures/faura/faura_1.jpg";
import faura_2 from "../assets/pictures/faura/faura_2.jpg";

import gate2_1 from "../assets/pictures/gate2/gate2_1.png";
import gate2_2 from "../assets/pictures/gate2/gate2_2.png";

import gate3_5_1 from "../assets/pictures/gate3.5/gate3_5_1.png";
import gate3_5_2 from "../assets/pictures/gate3.5/gate3_5_2.png";

import ags_1 from "../assets/pictures/ags/ags_1.png";
import ags_2 from "../assets/pictures/ags/ags_2.png";
import ags_3 from "../assets/pictures/ags/ags_3.png";

import beg_1 from "../assets/pictures/beg/beg_1.png";
import beg_2 from "../assets/pictures/beg/beg_2.png";
import beg_3 from "../assets/pictures/beg/beg_3.png";

import arete_1 from "../assets/pictures/arete/arete_1.png";
import arete_2 from "../assets/pictures/arete/arete_2.png";
import arete_3 from "../assets/pictures/arete/arete_3.png";

import covcourt_1 from "../assets/pictures/covcourt/covcourt_1.png";
import covcourt_2 from "../assets/pictures/covcourt/covcourt_2.png";
import covcourt_3 from "../assets/pictures/covcourt/covcourt_3.png";

import bel_1 from "../assets/pictures/bel/bel_1.png";
import bel_2 from "../assets/pictures/bel/bel_2.png";

import observatory_1 from "../assets/pictures/observatory/observatory_1.png";
import observatory_2 from "../assets/pictures/observatory/observatory_2.png";

import leong_1 from "../assets/pictures/leong/leong_1.png";
import leong_2 from "../assets/pictures/leong/leong_2.png";

import xavier_1 from "../assets/pictures/xavier/xavier_1.png";
import xavier_2 from "../assets/pictures/xavier/xavier_2.png";

import faber_1 from "../assets/pictures/faber/faber_1.png";
import faber_2 from "../assets/pictures/faber/faber_2.png";

import oldriz_1 from "../assets/pictures/oldriz/oldriz_1.png";
import oldriz_2 from "../assets/pictures/oldriz/oldriz_2.png";

import socsc_1 from "../assets/pictures/socsc/socsc_1.png";
import socsc_2 from "../assets/pictures/socsc/socsc_2.png";

import kostka_1 from "../assets/pictures/kostka/kostka_1.png";
import kostka_2 from "../assets/pictures/kostka/kostka_2.png";

import mvp_1 from "../assets/pictures/mvp/mvp_1.png";
import mvp_2 from "../assets/pictures/mvp/mvp_2.png";
import mvp_3 from "../assets/pictures/mvp/mvp_3.png";

import delacosta_1 from "../assets/pictures/delacosta/delacosta_1.png";
import delacosta_2 from "../assets/pictures/delacosta/delacosta_2.png";
import delacosta_3 from "../assets/pictures/delacosta/delacosta_3.png";

import newriz_1 from "../assets/pictures/newriz/newriz_1.png";
import newriz_2 from "../assets/pictures/newriz/newriz_2.png";

import berch_1 from "../assets/pictures/berch/berch_1.png";
import berch_2 from "../assets/pictures/berch/berch_2.png";

import schmitt_1 from "../assets/pictures/schmitt/schmitt_1.png";
import schmitt_2 from "../assets/pictures/schmitt/schmitt_2.png";

import gonz_1 from "../assets/pictures/gonz/gonz_1.png";
import gonz_2 from "../assets/pictures/gonz/gonz_2.png";

import pipac_1 from "../assets/pictures/pipac/pipac_1.png";
import pipac_2 from "../assets/pictures/pipac/pipac_2.png";

import seca_1 from "../assets/pictures/seca/seca_1.png";
import seca_2 from "../assets/pictures/seca/seca_2.png";

import secb_1 from "../assets/pictures/secb/secb_1.png";
import secb_2 from "../assets/pictures/secb/secb_2.png";

import secc_1 from "../assets/pictures/secc/secc_1.png";
import secc_2 from "../assets/pictures/secc/secc_2.png";

import ctc_1 from "../assets/pictures/ctc/ctc_1.png";
import ctc_2 from "../assets/pictures/ctc/ctc_2.png";
import ctc_3 from "../assets/pictures/ctc/ctc_3.png";

import som_1 from "../assets/pictures/som/som_1.png";
import som_2 from "../assets/pictures/som/som_2.png";

import jsec_1 from "../assets/pictures/jsec/jsec_1.png";
import jsec_2 from "../assets/pictures/jsec/jsec_2.png";
import jsec_3 from "../assets/pictures/jsec/jsec_3.png";

import macci_1 from "../assets/pictures/macci/macci_1.png";
import macci_2 from "../assets/pictures/macci/macci_2.png";

import gesu_1 from "../assets/pictures/gesu/gesu_1.png";
import gesu_2 from "../assets/pictures/gesu/gesu_2.png";

export const markers = [
{ geocode:[14.634666,121.0746], popUp:"Gate 1",
    note: "Campus Access Point ",
    description: "Campus access point for pedestrians, located near the Ateneo Grade School. Closest access point from LRT2-Katipunan station and terminal for UP Diliman-bound jeepneys. Enter here for easier access to Line A e-jeep services, the Ateneo Grade School, and the Blue Eagle Gym." ,
    images:
    [ gate1_1,
      gate1_2,
      gate1_3 ]
  },
  { geocode:[14.635807,121.074598], popUp:"Gate 2",
    note: "Campus Access Point",
    description: "Vehicular entry point only. For pedestrians, proceed to Gate 2.5, which is the best access point to enter/exit if coming from/to the vicinity of Shakey's Katipunan, or if coming from/to Metrolink services.",
    images:
    [gate2_1, gate2_2]
  },
  { geocode:[14.64053,121.074687], popUp:"Gate 3.5",
    note: "Campus Access Point",
    description: "Campus access point for pedestrians, mainly serving people to/from Regis Center. Enter here for easier access to the LS Complex via Leong Hall, Arete, Line B e-jeep services to Ateneo High School, and the Church of the Gesu",
    images:
    [gate3_5_1,gate3_5_2]
   },
  { geocode:[14.634463,121.076798], popUp:"Ateneo Grade School",
    note: "AGS",
    description: "Ateneo Grade School. The Henry Lee Erwin Theater is located there!",
    images:
    [ags_1,ags_2,ags_3]
   },
  { geocode:[14.63491,121.075553], popUp:"Blue Eagle Gym" ,
    note: "LS Campus",
    description: "The primary home court and training facility for the Ateneo Blue Eagles, hosting UAAP basketball, volleyball, and other varsity sports.",
    images:
    [beg_1,beg_2,beg_3]
  },
  { geocode:[14.641337,121.075958], popUp:"Arete Museum",
    note: "LS Campus",
    description: "Houses theaters (Hyundai Hall), galleries (Ateneo Art Gallery), studios, laboratories, and a makerspace (Eugenio Lopez, Jr. Makerspace). BS Rent students often have their culinary classes here. However, for most students, they often have their art appreciation classes here.",
    images:
    [arete_1,arete_2,arete_3]
  },
  { geocode:[14.636929,121.078565], popUp:"Covered Courts",
    note: "LS Campus",
    description: "During the daytime, most Physical Education classes are held here. At night, a lot of large scale gatherings, and events happen, such as organization events, or alumni reunion.",
    images:
    [covcourt_1,covcourt_2,covcourt_3]
  },
  { geocode:[14.641579,121.079373], popUp:"Bellarmine Hall",
    note: "LS Campus",
    description: "A former dormitory, converted to classroom space. Additional services include the Ateneo Bulatao Center for Psychology Services, and the Ateneo University Press store. Thus, the classes held here are usually but not limited to Histo, Theo, and Philo. There is usually no air conditioning in the classrooms.",
    images:
    [bel_1,bel_2]
  },
  { geocode:[14.636139,121.077589], popUp:"Manila Observatory",
    note: "LS Campus",
    description: "A Jesuit scientific research institution to study atmospheric and earth sciences—specifically climate change, disaster risk resilience, and environmental sustainability—to empower communities and guide decision-making through science, focusing on sustainable development.",
    images:
    [observatory_1,observatory_2]
  },
  { geocode:[14.640843,121.076296], popUp:"Leong Hall",
    note: "LS Complex",
    description: "A drop off point for cars to drop off or pick up students. At the ground floor, the auditorium often hosts large-scale events, seating 500 seats. As you go up the different floors,it houses faculty rooms and administrative offices for various Social Science departments. Home to the European Studies, Korean Studies, and Japanese Studies programs. Houses the Ricardo Leong Center for Chinese Studies, which fosters understanding of Chinese culture, history, and business, as well as the Ricardo Leong Institute for Global and Area Studies.",
    images:
    [leong_1,leong_2]
  },
  { geocode:[14.640116,121.078463], popUp:"Xavier Hall",
    note: "LS Complex",
    description: "Another drop off point for students to get picked up or dropped off to. It also contains a lot of offices, such as the Office of the Vice President for Higher Education, Office of University Development and Alumni Affairs, etc. Furthermore, if you lose anything, head over to window 13 in Xavier hall to retrieve your items.",
    images:
    [xavier_1,xavier_2]
  },
  { geocode:[14.640191,121.078045], popUp:"Faber Hall",
    note: "LS Complex",
    description: "Serves as a multipurpose facility housing the University Archives (the central repository for historical records), the Digital Information and Technology Services (DITS), and the Rizal Mini Theatre. It is used for academic, administrative, and wellness activities, including seminars and exhibits",
    images:
    [faber_1,faber_2]
  },
  { geocode:[14.640261,121.077291], popUp:"Old Rizal Library",
    note: "LS Complex",
    description: "Serves as a dedicated hub for preserving and providing access to rare Filipiniana materials, Philippine archives, and specialized research resources. It houses unique cultural, historical, and artistic documents, supporting in-depth academic research and the study of Philippine culture.",
    images:
    [oldriz_1,oldriz_2]
  },
  { geocode:[14.640619,121.076691], popUp:"Social Sciences Building",
    note: "LS Complex",
    description: "Have to go to the Office of Health Services for BINHI/PUNLA/BIGKIS Medical Clearance, or to undergo a drug test because you were randomly selected? This is the place to go! The registrar and other offices, such as the office of placement and career services (to help you find a j*b) is located there.",
    images:
    [socsc_1,socsc_2]
  },
  { geocode:[14.639757,121.078088], popUp:"Kostka Hall",
    note: "LS Complex",
    description: "A building inside the LS complex which contains classrooms and administrative offices. It is adjacent to Xavier Hall. Offices include the Information Desk, OAA, OAVP-UEd, and OAVP-GE. ATM machines are also located near Kostka.",
    images:
    [kostka_1,kostka_2]
  },
  { geocode:[14.639734,121.077704], popUp:"MVP Center",
    note: "LS Complex",
    description: "A building which contains A-Shop, administrative offices, and org rooms for COA organizations. It connects to Colayco Pavillion.",
    images:
    [mvp_1,mvp_2,mvp_3]
  },
  { geocode:[14.640009,121.076961], popUp:"Dela Costa Hall",
    note: "LS Complex",
    description: "Home to the famous Dela Costa statue, this building has the Philosophy, Theology, Filipino, and Foreign Language departments.",
    images:
    [delacosta_1,delacosta_2,delacosta_3]
  },
  { geocode:[14.640001,121.076243], popUp:"New Rizal Library",
    note: "LS Complex",
    description: "A five-story library which has computers that are open for use by all students, and which also serves as a student space.",
    images:
    [newriz_1,newriz_2]
  },
  { geocode:[14.63948,121.078303], popUp:"Berchmans Hall",
    note: "LS Complex",
    description: "A building for classrooms. Located near Gonzaga Hall.",
    images:
    [berch_1,berch_2]
  },
  { geocode:[14.639506,121.077004], popUp:"Faura Hall",
    note: "LS Complex",
    description: "Houses the Department of Information Systems and Computer Science (DISCS) and Department of Physics, as well as the Faura Audio-Visual Room.",
    images:
    [ faura_1,
      faura_2
    ]
  },
  { geocode:[14.639145,121.077578], popUp:"Schmitt Hall",
    note: "LS Complex",
    description: "Contains the Department of Chemistry, as well as chemistry labs. Located near Gonzaga Hall.",
    images:
    [schmitt_1,schmitt_2]
  },
  { geocode:[14.638963,121.078008], popUp:"Gonzaga Hall",
    note: "LS Complex",
    description: "A central cafeteria, hosting numerous dining options. It recently got renovated to include air conditioning and a more sleek internal design. However, the “smell” of all the oils can still stick to you.",
    images:
    [gonz_1,gonz_2]
  },
  { geocode:[14.638808,121.076784], popUp:"PIPAC",
    note: "LS Complex",
    description: "Philippine Institute of Pure and Applied Chemistry. Located next to PLDT-CTC.",
    images:
    [pipac_1,pipac_2]
  },
  { geocode:[14.638507,121.077576], popUp:"SEC A",
    note: "LS Complex",
    description: "A classroom building, home of the Department of Mathematics and Department of Biology. Connected to Secwalk and Skywalk.",
    images:
    [seca_1,seca_2]
  },
  { geocode:[14.638172,121.077197], popUp:"SEC B",
    note: "LS Complex",
    description: "Houses Biology labs. Connected to Secwalk and Skywalk, and is adjacent to SEC A and SEC C, as well as Matteo Ricci Hall.",
    images:
    [secb_1,secb_2]
  },
  { geocode:[14.638283,121.076953], popUp:"SEC C",
    note: "LS Complex",
    description: "Houses Physics labs, and is connected to Matteo Ricci Hall. Connected to Secwalk and Skywalk, and adjacent to SEC B and JGSOM.",
    images:
    [secc_1,secc_2]
  },
  { geocode:[14.638382,121.076733], popUp:"PLDT-CTC",
    note: "LS Complex",
    description: "A classroom building, which also contains the ECCE and Health Sciences departments. Adjacent to JGSOM, and near Faura Hall. Connected to Secwalk and Skywalk. Also an entry point to the LS complex.",
    images:
    [ctc_1,ctc_2,ctc_3]
  },
  { geocode:[14.638452,121.076422], popUp:"JGSOM",
    note: "LS Complex",
    description: "A classroom building, home to the John Gokongwei School of Management. Adjacent to PLDT-CTC and SEC C, and connected to Secwalk and Skywalk.",
    images:
    [som_1,som_2]
  },
  { geocode:[14.637892,121.076503], popUp:"JSEC",
    note: "LS Complex",
    description: "A dining area between SEC-C and CTC, with stalls featuring student JSEC challenge winners. Also an entry point for the LS Complex.",
    images:
    [jsec_1,jsec_2,jsec_3]
  },
  { geocode:[14.637596,121.076988], popUp:"Matteo Ricci Hall",
    note: "LS Complex",
    description: "A former student space, which is currently under renovation work. Located between SEC B and SEC C.",
    images:
    [macci_1, macci_2]
},
{ geocode:[14.6399375,121.0800545], popUp:"Church of the Gesu",
    note: "LS Complex",
    description: "Primarily holds regular Sunday masses at 6:00 PM, often featuring Jesuit presiders and school choirs. Special Masses, frequently for alumni, are held at 10:30 AM on the first and third Sundays of the month, and the church is open to the public.",
    images:
    [gesu_1,gesu_2]
},
]


export const ateneoBoundary = [
    [14.641585, 121.074739],
    [14.641025, 121.074700],
    [14.634565, 121.074533],
    [14.634538, 121.075393],
    [14.633976, 121.075326],
    [14.633671, 121.076487],
    [14.633245, 121.0771],
    [14.632782, 121.077583],
    [14.632767, 121.078152],
    [14.633559, 121.079527],
    [14.634558, 121.081651],
    [14.635126, 121.081999],
    [14.635783, 121.082013],
    [14.635875, 121.081762],
    [14.636317, 121.081562],
    [14.637544, 121.081872],
    [14.638349, 121.081963],
    [14.638474, 121.081516],
    [14.640999, 121.082668],
    [14.643927, 121.083681],
    [14.645318, 121.083507],
    [14.646143, 121.083059],
    [14.647046, 121.081257],
    [14.646176, 121.079953],
    [14.643853, 121.080112],
    [14.643721, 121.079717],
    [14.643009, 121.079706],
    [14.642756, 121.079858],
    [14.642253, 121.079183],
    [14.642180, 121.077768],
    [14.642176, 121.076099],
    [14.642330, 121.075651],
    
  ];